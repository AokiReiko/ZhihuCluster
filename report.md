...
由于原数据集规模庞大，本推荐系统所需的数据只有其中的一部分，故需要做数据清洗。
这部分的工作逻辑简单，是很容易并行处理的，每一个环节都是一个MapReduce。
...
考虑到传统聚类方法难以处理较大数据集的，我们采用先抽样，后使用k近邻来对用户分类。
其中的每一步都由MapReduce实现。

### 1. 向量生成

在提取用户特征的那一步得到了用户在不同话题下面的交互数，借助上一步生成的话题向量，可以将用户的兴趣表示成50维的向量，公式如下：

$u_{i} = \sum_{j\in{Topics}}ln(n_{j})*v_{j},         \,\,\,\,\, for \,\,  i \in{Users} $

n表示用户在某个话题下的交互数。即用户向量表示为话题向量的加权和。求完后需要对用户向量做归一化，因为加权之后距离已经没有意义，后面使用余弦衡量用户相似度。normalizedUserVec/ 中保存了归一化结果，格式为：
...
首先对用户采样，在活跃用户中抽取1000个样本做 k-means聚类，k取20。得到聚类结果后，以此作为knn的训练集。训练集保存在 marked/ 中，格式为：

````
UserID   \t   d1 d2 d2 ... d50 /Tag
````

Tag 是所属的类别的id，k-means 中心点保存在iter1-9/中。

### 3. K近邻分类

以1000个样本点作为训练集合，knn的参数k取训练集的平方根，即30。经过测试，样本点如果过多会导致分类很慢。map过程找到测试集中每个用户的所属类别，reduce过程输出 类别id—>用户id。由于每个类别下面用户很多，所以不将同一类用户一起存放。
...
模拟一个产生新用户的情景，用户在完成创建后由系统引导选择自己的偏好，并由此得到推荐。

### 选择感兴趣的话题

系统随机一组话题列表，让用户选择自己感兴趣的一些话题，将之求和转化为前述50维描述用户兴趣的向量。

### 寻找相似的用户

对于当前的用户，在1000个样本点中找到相似度最高的k个样本，取其中重合度最高的类别作为该用户的所属类别，之后在该类别下的数据集中，再次寻找重合度最高的r个样本，得到与该用户兴趣相似的用户。

### 协同过滤

找到了重合度高的用户后，将这些用户的历史浏览数据作为推荐的参考信息，将这些数据取并集加入到待推荐的集合中，按答案的受欢迎度和新鲜度加权做一个概率分布，随机取出一些答案作为最终的推荐结果。
